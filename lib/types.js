...
const castToString = val => {
  ...
  if(isFunction(val)) throw new CastError(String, val);
  if(isArray(val)) throw new CastError(String, val);
  const makeString = String(val);
  if(!isString(makeString)) throw new CastError(String, val);
  return makeString;
};
...
const castToBoolean = val => {
  ...
  if(isFunction(val)) throw new CastError(Boolean, val);
  if(isArray(val)) throw new CastError(Boolean, val);
  if(isString(val)) throw new CastError(Boolean, val);
  if(isNumber(val) && val !== 0 && val !== 1) throw new CastError(Boolean, val);
